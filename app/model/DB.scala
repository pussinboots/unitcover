...
object DB {
  ...
  def dbConfigUrl: String = {
    val p = Properties.envOrElse("CLEARDB_DATABASE_URL", "mysql://root:mysql@127.0.0.1:3306/unitcover")
    println(p)
    p
  }
  ...
  def parseDbUrl(mysqlUrl: String = dbConfigUrl) = {
    ...
    val dbUrl = "jdbc:mysql://" + dbUri.getHost() + port + dbUri.getPath() + "?autoReconnectForPools=true&reconnect=true&useSSL=true&useUnicode=yes&characterEncoding=UTF-8"
